...
if __name__ == "__main__":
    ...
    # 스크립트 이름에 따라 다른 메인 함수 실행
    script_name = os.path.basename(sys.argv[0])

    if 'backup' in script_name and 'restore' not in script_name:
        exit(backup_main())
...
def main():
    ...
    parser.add_argument(
        "--config", "-c",
        default="config/db_config.json",  # !/usr/bin/env python3
        """
        백업 및 복원 스크립트
...
class BackupRestoreManager:
    ...
    def _collect_metadata(self) -> Dict[str, Any]:
        ...
        try:
            ...
            version_result = self.db_manager.execute_query